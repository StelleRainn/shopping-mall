...
<script>
import { getAddressListApi, setDefaultAddressApi } from '@/api/address'
...
export default {
  ...
  methods: {
    // 为测试账号提供默认地址（确保页面可以渲染）
    async setDeafaultAddress () {
    ...
    // 获取订单信息(区分从购物车来和从立即购买来)
    async getOrderList () {
      if (this.mode === 'cart') {
        const res = await checkOrderApi(this.mode, {
